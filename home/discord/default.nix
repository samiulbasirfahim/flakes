...
let
  pywal_discord = pkgs.writeShellScriptBin "pywal-discord" ''
    #!/bin/env bash
    cat "$HOME/.local/share/discord_tui.css" "$HOME/.cache/wal/discord.css" > "$HOME/.config/Vencord/themes/tui.css" 
  '';

in
{

  home.file.".local/share/discord_tui.css".text = ''
    /**
     * @name Spotify tui theme like discord theme
